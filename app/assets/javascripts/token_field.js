...
const TokenField = function(input, url, options = {}, callback = null) {
    ...
    function addToken(text, value) {
        ...
        if (options.multiple) {
            ...
            if($("input:hidden[name='booth[vendor_id]']").length == 1){
              autoSelectRewards(value);
            }
        }
    ...
    function clearHandler() {
        if (options.multiple) {
          ...
          // remove auto-slect value of discount for booth
          if($("input:hidden[name='booth[vendor_id]']").length == 1){
            var discount = id('fee_discount');
            discount.value = "";
          }
            this.remove();
            hidden.remove();
    ...
    // Auto-select discount status for booth
    function autoSelectRewards(value) {
      ajaxWithStatus('GET', '/auto_select?vendor_id=' + value, {}, function() {
        var resp = JSON.parse(this.response);
        if(resp["vendor"] == true){
          var discount = id('fee_discount');
          discount.value = "rewards";
        }
      });
    }
};
